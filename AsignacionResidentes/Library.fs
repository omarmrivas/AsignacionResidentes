...
let leerProfesores (filePath:string) : (string * int * int * int) [] = 
    seq {
        use sr = new StreamReader (filePath)
        while not sr.EndOfStream do
            yield Sscanf.sscanf "%s,%i,%i,%i" (sr.ReadLine ())
    } |> Seq.toArray

let leerAsignaciones (filePath:string) = 
    let parse_matricula_plan str = 
        let pattern = @"(\((?<matricula>\w+),(?<plan>\w+)\))+"
        let regex = new Regex(pattern)
        [ for m in regex.Matches str do
            yield (m.Groups.["matricula"].Value, m.Groups.["plan"].Value) ]
    seq {
        use sr = new StreamReader (filePath)
        while not sr.EndOfStream do
            yield (sr.ReadLine ())
                    |> Sscanf.sscanf "%s,%i,<%s>,<%s>" 
                    |> (fun (s,id, asesores, revisores) -> 
                            (s,id, parse_matricula_plan asesores,
                                   parse_matricula_plan revisores))
    } |> Seq.toArray
